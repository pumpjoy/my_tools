...
class Example(QMainWindow):
    def __init__(self):
        ...
        self.jpg_dropdown.addItem("JPG/PNG Resize Batch to Small (MB)")
        ...
        ##########################
        # Stack: JPG/PNG-Batch Size Resizer 
        # Select jpg/png image files, input a number (by MB). The image sizes will be reduced to below the number. 
        self.jpng_batch_resizer_page = QWidget()
        jpng_batch_resizer_label1 = QLabel("This is JPG/PNG Batch Resizer. Select jpg/png image files, input a number (by MB). The image sizes will be reduced to below the number.")
        jpng_batch_resizer_label2 = QLabel("")
        jpng_batch_resizer_label3 = QLabel("")
        jpng_batch_resizer_button1 = QPushButton("Select Multiple JPG/PNG Files", self)
        jpng_batch_resizer_button1.clicked.connect(lambda: self.select_file(jpng_batch_resizer_label3, 'files', 'Image Files (*jpg *jpeg *png)'))
        
        self.jpng_batch_resizer_line_edit1 = QLineEdit()
        jpng_batch_resizer_button2 = QPushButton("Resize to smaller", self)
        jpng_batch_resizer_button2.clicked.connect(lambda: self.jpng_batch_size_resizer(jpng_batch_resizer_label2))
        
        self.jpng_batch_resizer_layout = QVBoxLayout()
        self.jpng_batch_resizer_layout.addWidget(jpng_batch_resizer_button1)
        self.jpng_batch_resizer_layout.addWidget(jpng_batch_resizer_label1)
        self.jpng_batch_resizer_layout.addWidget(self.jpng_batch_resizer_line_edit1)
        self.jpng_batch_resizer_layout.addWidget(jpng_batch_resizer_button2)
        self.jpng_batch_resizer_layout.addWidget(jpng_batch_resizer_label2)
        self.jpng_batch_resizer_layout.addWidget(jpng_batch_resizer_label3)
        
        self.jpng_batch_resizer_layout.addStretch()
        self.jpng_batch_resizer_page.setLayout(self.jpng_batch_resizer_layout)
        ...
        self.jpg_stack.addWidget(self.jpng_batch_resizer_page)
...
############################################################
    ...
    * Label to determine which label to change text
    ...
    """
    JPG/PNG Batch size resizer
    @param
        max_size_mb = 1 - defaults to maximum 1MB 
    """
    def jpng_batch_size_resizer(self, label, shrink_factor=0.9, max_size_mb=1): 
        # Create new folder in self.list_of_file_paths to store all new images
        if not self.list_of_file_paths:
            label.setText("Please select image files first.")
            return

        # Check if there are convertible files in the list
        user_input = self.jpng_batch_resizer_line_edit1.text()
        
        # Defaulting non-valid input or negative numbers to 1 MB
        try:
            max_size_mb = int(user_input)
        except ValueError:
            label.setText("Invalid input. Please enter a valid number.")
            return

        if max_size_mb <= 0:
            label.setText("Size must be greater than zero.")
            return

        # Make new folder to save resized images
        file_path = self.list_of_file_paths[0]
        new_folder = os.path.join(os.path.dirname(file_path), 'resized')
        ...
        for file in self.list_of_file_paths:
            ...
            # If the file size is within the allowed limit, no resizing is needed
            if file_size <= max_size_mb: 
                label.setText(f"File {file} already meets size requirement.")
                continue
            ...
            while file_size > max_size_mb:
                ...
                image = image.resize((new_width, new_height), Image.LANCZOS)  # High-quality downsampling
                
                # Save the resized file in the new folder with a new name to avoid overwriting
                file_name = os.path.basename(file).split('.')[0] + '_resized.jpg'  # Add '_resized' and set format
                ...
                if new_width < 200 or new_height < 200:
                    label.setText(f"Image {file} is too small to shrink further.")
                    break
